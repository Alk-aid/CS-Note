...
**函数引用返回不创建局部变量，直接返回变量本身，使函数调用可以作为左值**
...
**引用可以使原本只能在某个函数中使用的变量，换个别名在另一个函数使用**
...
**注解**：**非静态成员函数不属于类的对象上，因为类中的对象都可以调用**

​			**空对象占用的内存空间是 1 字节（目的：为了区分空对象占内存的位置）**

​			**对象所占的内存空间是由非静态成员变量所决定的**
...
**注释：本质上是把this指针改成 const Person * const this**
