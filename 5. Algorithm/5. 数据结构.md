...
- 首先题目要求get和put的复杂度都为O(1)；
- 我们首先可以想到使用HashMap，这样get一定是O(1)的；但是因为LRU还要有顺序之分，区分最近使用的和久未使用的数据；
- 所以的话还需要借助于链表，同时因为其中有删除操作，使用双向链表更为的方便
- 因此我们结合使用HsahMap 和 LinkedList
...
什么时候使用单调栈：一维数组，要寻找**任一个元素的右边或者左边第一个比自己大或者小的元素的位置**

单调栈存放的元素：只需要存放元素的下标i，如果需要使用对应的元素，直接T[i]就可以获取。
